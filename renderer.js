...
function renderConnectionList() {
    ...
    connections.filter(c => c.name.toLowerCase().includes(searchStr) || c.host.toLowerCase().includes(searchStr))
      ...
      <button class="conn-delete-btn" title="删除连接">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
      </button>
    `;

            el.addEventListener('dblclick', () => openConnectionTab(conn.id));
            el.querySelector('.conn-delete-btn').addEventListener('click', async (e) => {
                e.stopPropagation(); // prevent dblclick trigger if double-click bubble happens
